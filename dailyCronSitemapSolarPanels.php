<?php
    ...
    $sitemapFile = "sitemapsolarpanels.xml";
    
    // Build up the new sitemap in a string
    $sitemap = sitemapHeader();
    
    $SQL = "SELECT * FROM parser_solar ORDER BY Manufacturer ASC, ModelNumber ASC";
    ...
    while ($manufacturer = mysqli_fetch_array($manufacturers, MYSQLI_ASSOC)) {
        ...
        $sitemap .= sitemapEntrySolarPanel($manufacturerEx, $modelEx);
    }
    ...
    function sitemapEntrySolarPanel($manufacturer, $model) {
    	global $siteURLSSL;
    	
		$entry = '<url>';
		$entry .= '<loc>' . $siteURLSSL . 'panels/' . $manufacturer . '/' . $model . '.html</loc>';
